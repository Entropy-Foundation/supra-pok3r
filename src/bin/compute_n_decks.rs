...
use pok3r::aws::{
    ...
    read_deck_counter, read_deck_s3, read_deck_s3_node, save_deck_s3, save_deck_s3_node,
};
...
async fn main() {
    ...
    // check s3 bucket
    //loop {
    let decks = list_files_shallow_s3(&s3_client, "test_key/")
        .await
    ...
    while deck_no <= last_deck {
        ...
        */
        ...
        let deck_name_local = format!("/home/ec2-user/decks/deck_{deck_no}.bin");
        ...
        save_deck_s3_node(&s3_client, deck_no, ctxt.clone(), proof.clone()).await;
        ...
        let (deck_aws, proof_aws) =
            read_deck_s3_node(&s3_client, deck_no, &format!("{instance_id}"))
                .await
