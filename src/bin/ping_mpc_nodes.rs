...
async fn main() {
    ...
    kick_workers_multi(&ssm_client, tag_key, tag_val, starting_deck, num_decks)
        .await
    ...
    let deck_i = read_deck_s3_node(&s3_client, starting_deck, id)
        .await
        .expect("failed to read deck from s3");
    let deck_f = read_deck_s3_node(&s3_client, starting_deck + num_decks - 1, id)
        .await
        .expect("failed to read deck from s3");
    /*
    loop {
...
pub async fn kick_workers_multi(
    ...
    let cmd = format!("/home/ec2-user/pok3r/compute_n_decks --starting-deck {starting_deck} --num-decks {num_decks}");
